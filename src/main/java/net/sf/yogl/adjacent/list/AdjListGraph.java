...
public final class AdjListGraph <VK extends Comparable<VK>, VV, EK extends Comparable<EK>, EV> implements ImplementationGraph <VK, VV, EK, EV> {
	 ...
	 */
	Map<VK, AdjListVertex<VK, VV, EK, EV>> vertices = null;
	 ...
	 */
	public AdjListGraph(Map<VK, AdjListVertex<VK, VV, EK, EV>> prototype) throws GraphCorruptedException {
	...
	public void addNode(Vertex v) {
		if(!(v instanceof AdjListVertex)){
			throw new GraphException("Expected type: AdjListVertex" + v.getClass().getName());
		}
		AdjListVertex<VK, VV, EK, EV> c = (AdjListVertex<VK, VV, EK, EV>)v;
		this.addNode(c.getKey(), c.getUserValue());
	}
	...
	public void addNode(VK nodeKey, VV nodeValue)
		...
		if (!existsNode(nodeKey)) {
			AdjListVertex<VK, VV, EK, EV> rVertex = new AdjListVertex<>(nodeKey, nodeValue);
	...
	public void deepCopy(ComparableKeysGraph<VK, VV, EK, EV> dest) throws GraphException {
		...
		for(AdjListVertex<VK, VV, EK, EV> v : this.getVertices(0xFFFFFFFF)){
	...
	private VV findNodeByKey(VK nodeKey) throws NodeNotFoundException {
		...
		AdjListVertex<VK, VV, EK, EV> v = findVertexByKey(nodeKey);
	 ...
	 *            found in the graph.
	 */
	public AdjListVertex<VK,VV, EK, EV> findVertexByKey(VK nodeKey)
	...
	public EK[] getLinksKeysBetween(VK nodeFromKey, VK nodeToKey)
		...
		AdjListVertex<VK,VV, EK, EV> p = findVertexByKey(nodeFromKey);
		//retrieve all edges
		AdjListEdge<VK, EK, EV>[] eList = p.getNeighbors();
	...
	private void buildAllStartNodeKeys() {
		...
		while (entriesIter.hasNext()) {
			...
			AdjListVertex v = (AdjListVertex) entry.getValue();
			AdjListEdge[] successors = v.getNeighbors();
	...
	public List<Vertex> getRoots(){
		...
		for(AdjListVertex v: this.getVertices(VertexType.START | VertexType.STARTEND)){
			result.add(v);
		}
		return result;
	}
	...
	public EK[] getIncomingLinksKeys(VK nodeToKey)
		throws NodeNotFoundException {
		ArrayList<EK> result = new ArrayList<>();
		AdjListVertex<VK,VV,EK,EV> to = findVertexByKey(nodeToKey);
		Iterator<AdjListVertex<VK,VV,EK,EV>> iter = vertices.values().iterator();
		while (iter.hasNext()) {
			AdjListVertex<VK,VV,EK,EV> vertex = iter.next();
			AdjListEdge<VK,EK,EV>[] edge = vertex.getEdgeTo(nodeToKey);
			if (null != edge) {
	...
	public Object[][] getIncomingLinksKeysAndNodesKeys(VK nodeToKey)
		...
		Iterator<Map.Entry<VK, AdjListVertex<VK,VV,EK,EV>>> nodesIter = this.vertices.entrySet().iterator();
		while (nodesIter.hasNext()) {
			Map.Entry<VK, AdjListVertex<VK,VV,EK,EV>> entry = nodesIter.next();
			AdjListVertex<VK,VV,EK,EV> vertex = entry.getValue();
			AdjListEdge<VK,EK,EV>[] edges = vertex.getEdgeTo(nodeToKey);
	...
	public List<AdjListVertex<VK,VV,EK,EV>> getVertices(int nodeType) throws GraphCorruptedException {
		...
		ArrayList<AdjListVertex<VK,VV,EK,EV>> list = new ArrayList<>();
		try {
			Iterator<Map.Entry<VK, AdjListVertex<VK,VV,EK,EV>>> iter = vertices.entrySet().iterator();
			while (iter.hasNext()) {
				Map.Entry<VK, AdjListVertex<VK,VV,EK,EV>> entry = iter.next();
				VK key = entry.getKey();
				AdjListVertex<VK,VV,EK,EV> vertex = entry.getValue();
	...
	public int getNodeType(VK nodeKey) throws NodeNotFoundException {
		if (this.allStartNodeKeys.contains(nodeKey)) {
			AdjListVertex<VK, VV, EK, EV> vertex = findVertexByKey(nodeKey);
	...
	public VV getNodeValue(VK nodeKey) {
		AdjListVertex<VK, VV, EK, EV> v = this.vertices.get(nodeKey);
		if (v == null)
			return null;
		return v.getUserValue();
	}
	...
	public Object[][] getOutgoingLinksKeysAndNodesKeys(VK nodeKeyFrom)
		throws GraphException {
		AdjListVertex<VK,VV,EK,EV> v = this.findVertexByKey(nodeKeyFrom);
		AdjListEdge<VK,EK,EV>[] edges = v.getNeighbors();
	 ...
	 */
	public List<AdjListVertex<VK, VV, EK, EV>> getSuccessorVertices(AdjListVertex<VK, VV, EK, EV> node)
		throws NodeNotFoundException {
		AdjListEdge<VK, EK, EV>[] edges = node.getNeighbors();
		List<AdjListVertex<VK, VV, EK, EV>>result = new ArrayList<>();
		for (int i = 0; i < edges.length; i++) {
	...
	public AdjListVertex<VK,VV,EK,EV>[] getSuccessorVertices(AdjListVertex<VK,VV,EK,EV> vertex, AdjListEdge<VK, EK, EV> link)
		throws NodeNotFoundException {
		ArrayList<AdjListVertex<VK,VV,EK,EV>> result = new ArrayList<>();
		AdjListEdge<VK,EK,EV>[] eList = vertex.getNeighbors();
		if (eList != null) {
			...
			for (int i = 0; i < eList.length; i++) {
				...
				if ((link == null)
					...
					AdjListVertex<VK,VV,EK,EV> v1 = findVertexByKey(eList[i].getNextVertexKey());
					result.add(v1);
				}
			}
		}
		return result.toArray(new AdjListVertex[result.size()]);
	}
	...
	public void addLinkFirst(
		...
		try {
			//create the new edge
			AdjListEdge<VK, EK, EV> newEdge = new AdjListEdge<>(linkKey, nodeKeyTo, linkValue);
			//retrieve all edges connected to node1
			AdjListVertex<VK, VV, EK, EV> vertexFrom = findVertexByKey(nodeKeyFrom);
			AdjListEdge<VK, EK, EV>[] eList1 = vertexFrom.getNeighbors();
	...
	public Collection<VV> nodesValues() {
		...
		Iterator<AdjListVertex<VK, VV, EK, EV>> vertexIter = vertices.values().iterator();
		while (vertexIter.hasNext()) {
			AdjListVertex<VK, VV, EK, EV> vertex = vertexIter.next();
	...
	public Collection<EV> linksValues() {
		...
		Iterator<AdjListVertex<VK, VV, EK, EV>> verticesIter = vertices.values().iterator();
		while (verticesIter.hasNext()) {
			AdjListVertex<VK, VV, EK, EV> vertex = verticesIter.next();
			AdjListEdge<VK, EK, EV>[] edges = vertex.getNeighbors();
			for (int i = 0; i < edges.length; i++) {
	...
	public void removeAllLinksBetween(VK nodeKeyFrom, VK nodeKeyTo)
		throws GraphException {
		AdjListVertex<VK, VV, EK, EV> from = this.findVertexByKey(nodeKeyFrom);
		AdjListEdge<VK, EK, EV>[] edges = from.getEdgeTo(nodeKeyTo);
		if (edges != null) {
			for (int i = 0; i < edges.length; i++) {
	...
	public void removeLink(
		...
		AdjListVertex<VK, VV, EK, EV> vertexFrom = findVertexByKey(nodeFromKey);
	...
	public void removeNode(VK nodeKey) throws NodeNotFoundException {
		...
		AdjListVertex<VK, VV, EK, EV> v = this.findVertexByKey(nodeKey);
		this.numberOfEdges -= v.getCountEdges();
		//remove all links to the node
		Iterator<AdjListVertex<VK, VV, EK, EV>> vertexIter = vertices.values().iterator();
		while (vertexIter.hasNext()) {
			AdjListVertex<VK, VV, EK, EV> vertex = vertexIter.next();
			AdjListEdge<VK, EK, EV>[] edges = vertex.getEdgeTo(nodeKey);
			if (edges != null) {
				for (int i = 0; i < edges.length; i++) {
	...
	public void setAllVisitCounts(int count) throws GraphException {
		Iterator<AdjListVertex<VK, VV, EK, EV>> nodeValuesIter = this.vertices.values().iterator();
		while (nodeValuesIter.hasNext()) {
			AdjListVertex<VK, VV, EK, EV> vertex = nodeValuesIter.next();
	...
	public EV getOutgoingLinkValue(VK nodeKeyFrom, EK linkKey)
		throws NodeNotFoundException {
		EV result = null;
		AdjListVertex<VK, VV, EK, EV> vertex = this.findVertexByKey(nodeKeyFrom);
		AdjListEdge<VK, EK, EV> edge = vertex.getEdge(linkKey);
		if (edge != null)
